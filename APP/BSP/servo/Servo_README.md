...
| 舵机型号 | DS3115 |
| PWM 定时器 | TIM1 |
| Yaw 舵机 | TIM1_CH4，PE14 |
| Pitch 舵机 | TIM1_CH3，PE13 |
| PWM 频率 | 50 Hz |
| TIM1 输入时钟 | 162 MHz |
| PSC | 179 |
| 计数频率 | 900 kHz |
...
2026-07-21 实机复测发现：在旧补偿下，软件角运动到 Yaw=-3°、Pitch=-7° 时装置才正确对正。驱动已将这两个软件偏移并入最终 PWM 零位补偿：
...
补偿不修改 `Servo_State_t` 中的逻辑目标角和当前角，所以云台初始化的软件角仍为 0°。更换舵机臂或重新安装后，应重新使用水平仪测量并调整这两个宏。
