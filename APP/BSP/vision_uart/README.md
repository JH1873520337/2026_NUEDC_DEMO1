...
`VisionUart_Start()` is called by `Serve_VisionControl_Init()` after USART1
initialization. The global HAL UART callbacks are dispatched in `Core/Src/usart.c`,
so USART1 vision reception and the UART5 screen can operate at the same time.
A service can consume the decoded snapshots as follows:
